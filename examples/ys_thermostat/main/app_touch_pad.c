...
#define TOUCH_INTERRUPT_THRESH_PERCENT  (80)
        ...
        for (int i = 0; i < TOUCHPAD_COUNT; i++) {
            if (s_pad_obj[i].activated == true) {
                if(s_pad_obj[i].activated_time){
                    ...
                    ESP_LOGD(TAG, "T%d activating continuous %ld ms", s_pad_obj[i].num, interval);
                    if(s_pad_cfg.press_keep_cb){
                        s_pad_cfg.press_keep_cb(s_pad_obj[i].num, interval);
                    }
                }else{
                    s_pad_obj[i].activated_time = esp_log_timestamp();
                    ESP_LOGI(TAG, "T%d activated %ld ms!", s_pad_obj[i].num, s_pad_obj[i].activated_time);
                    if(s_pad_cfg.press_start_cb){
                        s_pad_cfg.press_start_cb(s_pad_obj[i].num);
                    }
                }
                // Wait a while for the pad being released
                vTaskDelay(ACTIVATED_DELAY_MS / portTICK_PERIOD_MS);
                ...
                if(s_pad_obj[i].activated_time){
                    uint32_t keep_time = esp_log_timestamp() - s_pad_obj[i].activated_time - ACTIVATED_DELAY_MS - 10;
                    ESP_LOGI(TAG, "T%d total keep activated %ld ms", s_pad_obj[i].num, keep_time);
                    s_pad_cfg.press_end_cb(s_pad_obj[i].num, keep_time);
...
static void tp_example_rtc_intr(void *arg)
{
    ...
    for (int i = 0; i < TOUCHPAD_COUNT; i++) {
        if ((pad_intr >> s_pad_obj[i].num) & 0x01) {
...
void app_touch_pad_init(app_touch_pad_cfg_t *pCfg)
{
    ...
    if(!pCfg->press_end_cb){
        ESP_LOGE(TAG, "init fail, null press_end_cb !!!");
        return;
    }
